...
#include "L726_countOfAtoms.h"

string L726_countOfAtoms::countOfAtoms(string formula) {
    ...
    while (i < n) {
        ...
        if (ch == '(') {
            ...
            for (auto &[atom, v]:atomNum) {
                stk.top()[atom] += v*num; // 将括号内的原子数量成上num，加到上一层的原子数量中
            }
        } else {
